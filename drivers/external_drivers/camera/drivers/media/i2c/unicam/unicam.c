...
/* Add OTP operation
*/
//modify by zhanghf :for xinyusheng module ;
//it is different from f1018 for zhongliancheng
//#define BG_RATIO_TYPICAL  0x142
//#define RG_RATIO_TYPICAL  0x16C
static int BG_Ratio_Typical_OV5648 = 0x142;
static int RG_Ratio_Typical_OV5648 =  0x16C;
	    ...
	    int index, struct otp_struct *otp_ptr)
{
    ...
    vendor_id = ((*otp_ptr).module_integrator_id )& 0x007f ;
   
   pr_err("zhanghf %s vendor_id 0x(%x)  \n",__func__,vendor_id);

    if(vendor_id == 0x15){//for xinyusheng 
         BG_Ratio_Typical_OV5648 = 0x152;
         RG_Ratio_Typical_OV5648  = 0x14e;
    }else{ //for zhongliancheng 
         BG_Ratio_Typical_OV5648 = 0x142;
         RG_Ratio_Typical_OV5648 =  0x16c;
   }
...
static int ov5648_update_otp(struct v4l2_subdev *sd)
{
	...
	pr_err("zhanghf _ov5648_: %s :rg:0x%x (0x%x) bg 0x%x (0x%x)\n", __func__, rg,current_otp.rg_ratio, bg,current_otp.bg_ratio);
	if (rg == 0)
		rg = 1;
	if (bg == 0)
		bg = 1;
    ...
    pr_err("zhanghf after _ov5648_: %s :g_gain:%x (%x) b_gain %x (%x)\n", __func__, g_gain,RG_Ratio_Typical_OV5648,b_gain,BG_Ratio_Typical_OV5648);
...
static int det_id(struct i2c_client *client)
{
	...
	if (dev->uni->i2c_addr == 0x3C && !strncmp(dev->uni->desc, "gc2355", 6))
	{	
	...
	pr_err("zhanghf ov5648 det_id 0x%x need 0x%x\n",val,dev->uni->idval);
...
static int uni_s_ctrl(struct v4l2_subdev *sd, struct v4l2_control *ctrl)
{
	...
	if ((octrl == NULL) || (octrl->tweak == NULL))
		return -EINVAL;
pr_err("zhanghf uni_s_ctrl id=0x%08x ,name=%s value=%d \n",ctrl->id,octrl->qc.name,ctrl->value);
	mutex_lock(&dev->input_lock);
...
//	S_UNI_DEVICE *fwdev=NULL;
	...
	pr_err("zhanghf uni_probe name: %s, unicam_data = %d \n",client->name, unicam_data);
...
*/
/*If the rear camera didn't use the UNICAM machanism, 
but the front onedid, then uni_data couldn't point to m_data_front[] 
and the detectionwill be failed.
modify by intel luxingjiang
*/

    if (client->name && !strncmp(client->name, "front", 5))     
        uni_data = m_data_front;
    else
        uni_data = m_data;
