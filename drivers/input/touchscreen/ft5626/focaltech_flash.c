 ...
 struct fts_Upgrade_Info fts_updateinfo[] =
{
	...
	{0x54,FTS_MAX_POINTS_5,AUTO_CLB_NONEED,2, 2, 0x54, 0x2c, 20, 2000}, //,"FT5x46"
         {0x58,CFG_MAX_TOUCH_POINTS,AUTO_CLB_NONEED,2, 2, 0x58, 0x2c, 20, 2000},//"FT5822",
...
dev_err(&client->dev, "Change to STDI2cValue,REG1 = 0x%x,REG2 = 0x%x,REG3 = 0x%x, iRet=%d\n", auc_i2c_write_buf[0], auc_i2c_write_buf[1], auc_i2c_write_buf[2]);
	if(0xeb==auc_i2c_write_buf[0] && 0xaa==auc_i2c_write_buf[1] && 0x08==auc_i2c_write_buf[2])
	{
		pr_info("HidI2c_To_StdI2c successful.\n");
			dev_err(&client->dev, "HidI2c_To_StdI2c successful  \n");
	...
	else 
	{
		pr_err("HidI2c_To_StdI2c error.\n");
					dev_err(&client->dev, "HidI2c_To_StdI2c error  \n");
...
void fts_get_upgrade_array(void)
{
	...
	printk("%s chip_id = %x\n", __func__, chip_id);
	dev_err(&fts_i2c_client->dev, "++++fts_get_upgrade_array ++++,chip_id=0x%x \n", chip_id);
