...
static struct proc_dir_entry *gt91xx_status_switch_proc = NULL;  ////xmwuwh @20141201 add for tp gesture gesture switch start!
...
/*xmlinxs@20191101 support backlight controled by apk. */
#ifdef CONFIG_BACKLIGHT_APK_CONTROL_SUPPORT
static ssize_t backlight_ctrl_write_proc(struct file *file,const char __user *buffer, size_t count, loff_t *ppos);
static ssize_t backlight_ctrl_read_proc(struct file *file,char __user *buffer, size_t count, loff_t *ppos);
extern int lpio_bl_write_bits(uint8_t pwm_num, uint32_t reg, uint32_t val,
		uint32_t mask);
extern int lpio_bl_update(uint8_t pwm_num, uint32_t reg);
int backlight_ctrl_flag = 1;
int bl_brightness_vlaue;
extern int old_bl_level;
static struct proc_dir_entry *lcdbl_ctrl_proc;
static const struct file_operations bl_control_proc_fops = {
    .write = backlight_ctrl_write_proc,
    .read  = backlight_ctrl_read_proc
};
#endif
...
{
    s32 ret = 0;
    if (copy_from_user(&gt9271_config[0], buffer, count))
    {
        printk("copy from user fail\n");
        return -EFAULT;
    }
    printk("gt91xx_status_switch_write_proc, content: %d count %d\n", gt9271_config[0],count);
    if (gt9271_config[0] == '0')
        justResume_status = 0;
    else
        justResume_status = 1;
    return count;
...
#endif
/*xmlinxs@20191101support backlight controled by apk. -begin-*/
#ifdef CONFIG_BACKLIGHT_APK_CONTROL_SUPPORT
static ssize_t backlight_ctrl_read_proc(struct file *file, char __user *buffer, size_t count, loff_t *ppos)
{
    char page[16];
...
static ssize_t backlight_ctrl_write_proc(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    pr_info("[%s]mode = %d\n",__func__,mode);
    if(mode == 0){
        bl_brightness_vlaue = old_bl_level;
        GTP_INFO("bl_brightness = %d\n",bl_brightness_vlaue);
        lpio_bl_write_bits(0, LPIO_PWM_CTRL, (0xff - 0), 0xFF);
		lpio_bl_update(0, LPIO_PWM_CTRL);
        backlight_ctrl_flag = 0;
        return count;
    }else{
        GTP_INFO("bl_brightness = %d\n",bl_brightness_vlaue);
        lpio_bl_write_bits(0, LPIO_PWM_CTRL, (0xff - bl_brightness_vlaue), 0xFF);
		lpio_bl_update(0, LPIO_PWM_CTRL);
        backlight_ctrl_flag = 1;
        return count;
    }
    GTP_INFO("Wrong incoming parameter! echo 0 or 1\n");
    return -EINVAL;
}
#endif
/*xmlinxs@20191101 support backlight controled by apk. -begin-*/
...
void gtp_irq_enable(struct goodix_ts_data *ts)
{
    ...
    if (ts->irq_is_disable) 
    {
        irq_set_irq_type(ts->client->irq, IRQ_TYPE_EDGE_FALLING);  // xmtdf
...
#endif
/*xmlinxs@20191101support backlight controled by apk. */
#ifdef CONFIG_BACKLIGHT_APK_CONTROL_SUPPORT
    if(backlight_ctrl_flag == 0){
            GTP_INFO("turn on backlight!! level = %d\n",bl_brightness_vlaue);
            lpio_bl_write_bits(0, LPIO_PWM_CTRL, (0xff - bl_brightness_vlaue), 0xFF);
		    lpio_bl_update(0, LPIO_PWM_CTRL);
            backlight_ctrl_flag = 1;
        }
...
void gtp_int_sync(s32 ms)
{
    msleep(5); // add 5ms delay for sync by xmwuwh@20160118
...
void gtp_reset_guitar(struct i2c_client *client, s32 ms)
{
    ...
    pr_err("i2c addr retry times = %d i2c_addr: %x",retry_times,client->addr);
#endif
...
#if 0//(defined(CONFIG_TOUCHSCREEN_FT5X0X) && defined(CONFIG_TOUCHSCREEN_GOODIX_GT9XX)) //change by xmwuwh@20160118
	gt9xx_power_on(1);
//#else
	gt9xx_power_on(0);
	gt9xx_power_on(1);
    ...
    GTP_GPIO_OUTPUT(GTP_INT_PORT, client->addr == 0x14); // change for time sync by xmwuwh@20160118
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    while(retry++ < 10)
    {
        ...
        msleep(5);
    #endif
        gtp_reset_guitar(ts->client, 20);  //add TP reset avoid TP work failed by xmwuwh@20160113
        ret = gtp_i2c_test(ts->client);
        if (ret > 0)
        {
            GTP_DEBUG(" GTP wakeup sleep. end \n");
...
//xmwuwh at 20141211 for E8005A 台冠触摸屏和天之域触摸屏兼容 start
#ifdef CONFIG_TOUCHSCREEN_E8005
...
#endif
//xmwuwh at 20141211 for E8005A 台冠触摸屏和天之域触摸屏兼容 end  
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
    //u8 test[6] = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
    u8 test[6] = {GTP_REG_HW_INFO >> 8, GTP_REG_HW_INFO & 0xff};
    u8 retry = 0;
    s8 ret = -1;
    u32 hw_info = 0;
    //u8 test1[6] = {GTP_REG_HW_INFO >> 8, GTP_REG_HW_INFO & 0xff};

    GTP_DEBUG_FUNC();
  
    while(retry++ < 5)
    {
        ret = gtp_i2c_read(client, test, sizeof(test));
        memcpy(&hw_info,&test[2],4);
        //printk("xmwuwh---ret=%d,hw_info=0x%x\n",ret,hw_info);
...
/*  
#ifdef CONFIG_TOUCHSCREEN_E8005           //for E8005A GT9217 change IRQ for RISING
...
/* Add early suspend and late resume for Goodix GT9xx touchscreen by xmwuwh@20160122 */
#ifdef CONFIG_TOUCHSCREEN_GOODIX_GT9XX
static ssize_t gt9xx_power_hal_suspend_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "%s\n",
		ts->gtp_is_suspend ? "suspend" : "resume");
}
					     ...
					     const char *buf, size_t count)
{
	if (!strncmp(buf, POWER_HAL_SUSPEND_ON, POWER_HAL_SUSPEND_STATUS_LEN))
		goodix_ts_early_suspend(dev);
	else
		goodix_ts_late_resume(dev);
...
static struct device_attribute dev_attr_power_HAL_suspend =
	__ATTR(power_HAL_suspend, S_IRUGO | S_IWUSR, gt9xx_power_hal_suspend_show, gt9xx_power_hal_suspend_store);
#endif
/* Add early suspend and late resume for Goodix GT9xx touchscreen by xmwuwh@20160122 */
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    GTP_INFO("xmwuwh GTP I2C Address: 0x%02x", client->addr);
    ...
    {
        GTP_ERROR("create_proc_entry %s failed\n", GT91XX_STATUS_SWITHC_PROC_FILE);
    }
    else
    {
    ...
    /*xmlinxs@20191101 support backlight controled by apk. */
#ifdef CONFIG_BACKLIGHT_APK_CONTROL_SUPPORT
    lcdbl_ctrl_proc = proc_create("backlight_ctrl", 0777, NULL, &bl_control_proc_fops);
	if (lcdbl_ctrl_proc== NULL)
		GTP_ERROR("create_proc_entry 'backlight_ctrl' failed");
#endif
#if GTP_ESD_PROTECT
    gtp_esd_switch(client, SWITCH_ON);
#endif
...
/* Add early suspend and late resume for Goodix GT9xx touchscreen by xmwuwh@20160122 */
       ...
       if (ret < 0) {
           pr_err(" xmwuwh unable to create suspend entry");
           return ret;
       }
       ret = register_power_hal_suspend_device(dev);
       if (ret < 0)
       {
            pr_err("xmwuwh unable to register for power hal");
            return ret;
       }
#endif
    /* Add early suspend and late resume for Goodix GT9xx touchscreen by xmwuwh@20160122 */

    /* xmtdf @ 2016-04-08 default suspend TP, to fix the bug: in poweroff charging state, the I2C5 can't
       enter S0I3 state, then the power consumption is higher.    -- begin  */
    /* xmtdf @ 2016-04-09 revert this change because it will make the TP can't touch in FTM mode */
    //msleep(10);
    //goodix_ts_early_suspend(dev);
    /* xmtdf @ 2016-04-08 -- end  */
...
static int goodix_ts_early_suspend(struct device *dev)
{
    ...
    pr_err("xmwuwh GTP System suspend.\n");
    ts->gtp_is_suspend = 1;
#if GTP_ESD_PROTECT
    gtp_esd_switch(ts->client, SWITCH_OFF);
#endif
...
#else
// Setting TP sleep first then disable IRQ avoid TP can't work sometimes by xmwuwh@20160130
    ret = gtp_enter_sleep(ts);
    if (ts->use_irq)
    {
        gtp_irq_disable(ts);
...
static int goodix_ts_late_resume(struct device *dev)
{
    ...
    //ts = container_of(h, struct goodix_ts_data, early_suspend);
    struct i2c_client *client = to_i2c_client(dev);
    struct goodix_ts_data *ts = i2c_get_clientdata(client);//GTP_DEBUG_FUNC();

    pr_err("xmtdf goodix_ts_late_resume\n");
...
//#endif
    ...
    .driver = {
        .name     = GTP_I2C_NAME,
        .owner    = THIS_MODULE,
        //.pm		= &gt9xx_pm_ops,       /* Add early suspend and late resume for Goodix GT9xx touchscreen by xmwuwh@20160122 */
...
static int __init goodix_ts_init(void)
{
    ...
    GTP_DEBUG_FUNC();   
    pr_err("GTP driver installing...222222222222222222");
