...
////add gsensor calibration for KXTJ2-1009 by xmwuwh @20160128
#define GSENSOR_CALIBRATION
#ifdef GSENSOR_CALIBRATION
int i = 0,index=0,data_len = 4,data_flag=0,data_sum=0;
int32_t    data_sum_x=0,data_sum_y=0,data_sum_z=0;
int32_t    data_raw[10][3]={0,0};
#endif
////add gsensor calibration for KXTJ2-1009 by xmwuwh @20160128
...
////add gsensor calibration for KXTJ2-1009 by xmwuwh @20160128
#ifdef GSENSOR_CALIBRATION
        if(data_sum<data_len)
        {
            data_raw[data_sum][0]= acceld->accel_data[acceld->axis_map_x];
            data_raw[data_sum][1]= acceld->accel_data[acceld->axis_map_y];
            data_raw[data_sum][2]= acceld->accel_data[acceld->axis_map_z];

            data_sum_x += acceld->accel_data[acceld->axis_map_x];
            data_sum_y += acceld->accel_data[acceld->axis_map_y];
            data_sum_z += acceld->accel_data[acceld->axis_map_z];
            data_sum++;
        }
        else
        {
            data_sum_x -= data_raw[data_flag][0];
            data_sum_y -= data_raw[data_flag][1];
            data_sum_z -= data_raw[data_flag][2];
            data_raw[data_flag][0]=acceld->accel_data[acceld->axis_map_x];
            data_raw[data_flag][1]=acceld->accel_data[acceld->axis_map_y];
            data_raw[data_flag][2]=acceld->accel_data[acceld->axis_map_z];
            data_flag++;
            data_sum_x += acceld->accel_data[acceld->axis_map_x];
            data_sum_y += acceld->accel_data[acceld->axis_map_y];
            data_sum_z += acceld->accel_data[acceld->axis_map_z];
            if(data_flag>=data_len)
                data_flag = 0;
            acceld->accel_data[acceld->axis_map_x] = data_sum_x/data_len;
            acceld->accel_data[acceld->axis_map_y] = data_sum_y/data_len;
            acceld->accel_data[acceld->axis_map_z] = data_sum_z/data_len;
            if(atomic_read(&acceld->accel_input_event) > 0) {
                input_report_abs(acceld->input_dev, ABS_X, acceld->accel_data[acceld->axis_map_x]);
                input_report_abs(acceld->input_dev, ABS_Y, acceld->accel_data[acceld->axis_map_y]);
                input_report_abs(acceld->input_dev, ABS_Z, acceld->accel_data[acceld->axis_map_z]);
