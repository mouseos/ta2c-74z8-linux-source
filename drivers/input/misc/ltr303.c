...
#define MAX_VAL					50000
	...
	//if ((ch0_val == 0) && (ch1_val > 50 )) { //Fix for data unstable when cover the lightsensor by xmwuwh@20160408
	//	value = lux_val_prev;
	//} else {
		if (gain == 1) {
	...
	//}  //Fix for data unstable when cover the lightsensor by xmwuwh@20160408
...
static int8_t als_enable_init(struct ltr303_data *ltr303)
{
	...
	rc = als_meas_rate_reg_setup(0x02, ltr303);
	if (rc < 0) {
...
static void sysfs_register_device(struct i2c_client *client) 
{
	...
	rc += device_create_file(&client->dev, &dev_attr_enable);    // fixed lightsensor can not work start by xmwuwh@20160314
...
// fixed lightsensor can not work start by xmwuwh@20160314
	///ret = sysfs_create_group(&client->dev.kobj,
    //                  &ltr303_attribute_group);
    //   if (ret < 0)
    //            goto err_ltr303_setup;
    // fixed lightsensor can not work end by xmwuwh@20160314
