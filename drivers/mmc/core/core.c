...
u32 mmc_select_voltage(struct mmc_host *host, u32 ocr)
{
    ...
    if (strcmp("mmc2", mmc_hostname(host)) == 0)
       pr_err("xmtdf %s, %s ocr: 0x%x\n",__func__,mmc_hostname(host),ocr); 
...
power_cycle:
	if (err) {
		pr_debug("xmtdf %s: Signal voltage switch failed, "
			"power cycling card\n", mmc_hostname(host));
		mmc_power_cycle(host, ocr);
	}
...
void mmc_power_up(struct mmc_host *host, u32 ocr)
{
	if (host->ios.power_mode == MMC_POWER_ON)
		return;
    //pr_err("xmtdf %s, ocr: 0x%x",__func__,ocr);
    ...
    if (strcmp("mmc2", mmc_hostname(host)) == 0)
        pr_err("xmtdf %s, %s vdd: 0x%x \n",__func__,mmc_hostname(host),host->ios.vdd);
