					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		ret = pmic_fg_get_vocv(info, &value);
		pr_info("[%s] POWER_SUPPLY_PROP_CAPACITY vocv = %d\n", __func__, value);
		if ( value < 3500 )
			vol_low_count ++;
		else
			vol_low_count = 0;
		...
		if( vol_low_count > 3 )
		{
			...
			if( value < 3400 ) {
				...
				ret = pmic_fg_get_current(info, &value);
				pr_info("[%s] POWER_SUPPLY_PROP_CAPACITY current = %d\n", __func__, value);
				if( value < 1000 )
					val->intval = 0;
			}
		}
...
static void pmic_fg_status_monitor(struct work_struct *work)
{
	...
	struct batt_props bat_prop;   	// add by  xmtjx to printk some log for battery status
	...
	*/
	...
	/*add by xmtjx to printk some log for battery status -- begin*/
	bat_prop.voltage_now = VOLTAGE_OCV(&info->bat) / 1000;
	bat_prop.current_now = CURRENT_NOW(&info->bat) / 1000;

	pr_info("[%s] BATTERY: status = %d, temperature = %d, present_health = %d,"
					" present_cap = %d, voltage_ocv= %ld, current_now= %ld\n",
					__func__, info->status, info->btemp,present_health,present_cap,
					bat_prop.voltage_now,bat_prop.current_now);
	/*add by xmtjx to printk some log for battery status -- end*/
...
/* modify by xmtjx to set calibration threshold.  begin*/
#if 0
...
#endif
/* modify by xmtjx to set calibration threshold.  end*/
...
static void pmic_fg_init_config_regs(struct pmic_fg_info *info)
{
	...
	/*
	 *Disable calibrating SOC-OCV curve always for x-power suggests,
	 *but it not to intel . 
	 *So we do not need to initialize the FG data again at every booting. 
	 *Because the data from acpi would be wrong .
	 *And when bios is updated ,ensure the config data can be updated from acpi .
	 *modify by xmtjx 
	 */
#if 0
...
#endif
	} else if ((ret & FG_CNTL_CAP_ADJ_EN) && !info->pdata->cdata.fco){
		dev_info(&info->pdev->dev,
	...
	ret = pmic_fg_reg_writeb(info, DC_FG_CNTL_REG, 0xe0);
	if (ret < 0)
		dev_err(&info->pdev->dev, "gauge cntl set fail:%d\n", ret);
...
static void pmic_fg_init_hw_regs(struct pmic_fg_info *info)
{
	...
	/*modify by xmtjx to set the charge end condition to 10% of CC*/
#if 0
