...
static struct atomisp_camera_cht_table cht_cam_comp_tab[] = {
    ...
    {"i2c-OVTI2688:00", "OVTI2688:00", "rear",  "i2c-rear:36",  0x36, 0x00},
		...
		u32 level, void *data, void **return_value)
{
	...
	for (i = 0; i < ARRAY_SIZE(cht_cam_comp_tab); i++) {
       // pr_err("zhanghf %s dev:%s match:%s \n",__func__,dev_name(&adev->dev),cht_cam_comp_tab[i].subdev_match_name);
...
static const struct gmin_cfg_var cht_camera_config_vars[] = {
	...
	{ "OVTI2680:00_CsiLanes",        "1",    "ov2680" },
    	...
    	/* For MRD front camera ov2680 with MRD default BIOS */
	{ "OVTI2688:00_CamClk",          "4",    "ov2688" },
	{ "OVTI2688:00_ClkSrc",          "0",    "ov2688" },
	{ "OVTI2688:00_CsiPort",         "1",    "ov2688" },
	{ "OVTI2688:00_CsiLanes",        "1",    "ov2688" },
...
static struct gmin_subdev *gmin_subdev_add(struct v4l2_subdev *subdev)
{
	...
	dev_info(dev, "suddev name =%s  drv_name %s \n", subdev->name,dev->driver->name);
...
int __init atomisp_unregister_acpi_devices(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(cht_cam_comp_tab); i++) {
		...
		if (dev) {
			...
			pr_info("%s(): unregister i2c_device: %s! \
